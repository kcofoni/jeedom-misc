...
class userFunction {
    ...
    public static function getKeyfobButtonText($_scene = 0) {
      ...
      public static function translateNDBEvent($_evtText) {
      	$cleanedEvtText = preg_replace('/\"/', '', $_evtText); // suppression des double quote au cas où
        // log::add('user_function', 'error', 'input text (after possible cleaning) is ::' . $cleanedEvtText . '::');
                       
        $evtTextTab = array(
            "NDB-missed_call" => "Appel manqué",
            "NDB-incoming_call" => "Appel entrant",
            "NDB-accepted_call" => "Appel accepté",
            "NDB-human" => "Personne détectée",
            "NDB-vehicle" => "Voiture détectée",
            "NDB-animal" => "Animal détecté",
            "NDB-rtc" => "Sonnette actionnée",
            "NDB-connection" => "Sonnette connectée",
            "NDB-disconnection" => "Sonnette déconnectée",
            "missed_call" => "Appel manqué",
            "incoming_call" => "Appel entrant",
            "accepted_call" => "Appel accepté",
            "human" => "Personne détectée",
            "vehicle" => "Voiture détectée",
            "animal" => "Animal détecté",
            "rtc" => "Sonnette actionnée",
            "connection" => "Sonnette connectée",
            "disconnection" => "Sonnette déconnectée",
        );

        $translatedText = "[". $cleanedEvtText . "]";

        if (array_key_exists($cleanedEvtText, $evtTextTab))
            $translatedText = $evtTextTab[$cleanedEvtText];
